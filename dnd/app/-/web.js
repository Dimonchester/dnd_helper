...
;
	($.$my_dnd_cards) = class $my_dnd_cards extends ($.$mol_card) {
		Description(){
			const obj = new this.$.$mol_text();
	...
	($mol_mem(($.$my_dnd_cards.prototype), "Description"));
...
(function ($) {
    var $$;
    (function ($$) {
        class $my_dnd_cards extends $.$my_dnd_cards {
            name(next) {
                return next ?? '';
            }
            description(next) {
        ...
        __decorate([
            $mol_mem
        ], $my_dnd_cards.prototype, "name", null);
        __decorate([
            $mol_mem
        ], $my_dnd_cards.prototype, "description", null);
        $$.$my_dnd_cards = $my_dnd_cards;
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
...
;
	($.$my_dnd_spells) = class $my_dnd_spells extends ($.$mol_list) {
		Spell_row(id){
			const obj = new this.$.$my_dnd_cards();
			(obj.name) = () => ((this.spell_name(id)));
			(obj.description) = () => ((this.spell_description(id)));
			return obj;
		}
		spell_rows(){
			return [(this.Spell_row(id))];
		}
		rows(){
			return (this.spell_rows());
		}
	};
	($mol_mem_key(($.$my_dnd_spells.prototype), "Spell_row"));
...
(function ($) {
    var $$;
    (function ($$) {
        class $my_dnd_spells extends $.$my_dnd_spells {
            spells_data() {
                return [
                    { id: 'fb', name: 'Fireball (Огненный шар)', description: 'Яркая вспышка вырывается из вашего указательного пальца в точку, выбранную вами в пределах дистанции, где и взрывается...' },
                    { id: 'mm', name: 'Magic Missile (Волшебная стрела)', description: 'Вы создаёте три светящиеся стрелы из магической энергии. Каждая стрела попадает в существо, которое вы можете видеть в пределах дистанции.' },
                    { id: 'sh', name: 'Shield (Щит)', description: 'Невидимый барьер из магической силы появляется и защищает вас. До начала вашего следующего хода вы получаете +5 к КД.' }
                ];
            }
            spell_rows() {
                return this.spells_data().map(spell => this.Spell_row(spell.id));
            }
            spell_name(id) {
                return this.spells_data().find(s => s.id === id)?.name ?? '';
            }
            spell_description(id) {
                return this.spells_data().find(s => s.id === id)?.description ?? '';
            }
        }
        __decorate([
            $mol_mem
        ], $my_dnd_spells.prototype, "spells_data", null);
        __decorate([
            $mol_mem
        ], $my_dnd_spells.prototype, "spell_rows", null);
        __decorate([
            $mol_mem_key
        ], $my_dnd_spells.prototype, "spell_name", null);
        __decorate([
            $mol_mem_key
        ], $my_dnd_spells.prototype, "spell_description", null);
        $$.$my_dnd_spells = $my_dnd_spells;
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
